...
#define WINDOW_TITLE "Texture Mapping"
...
bool LoadTexture( const char *file_name, GLuint *tex ) {
	...
	if ( !image_data ) {
		LogError("ERROR: could not load %s\n", file_name );
		return false;
	}

	// NPOT(Not Power of Two) check
	if ( ( x & ( x - 1 ) ) != 0 || ( y & ( y - 1 ) ) != 0 ) {
		LogError("WARNING: texture %s is not power-of-2 dimensions\n",
						 file_name );
	}
...
int main() {
	...
	//-----------LOAD TEXTURE------------------
	GLuint tex = 0;
	LoadTexture("skulluvmap.png", &tex);
